  ...
  for (i in seq_along(row_groups)) {
    ...
    resp <- tryCatch(
      ...
      httr2_http_error = function(e) {
        body_txt <- tryCatch({
          b <- httr2::resp_body_json(e$resp, check_type = FALSE)
          paste(
          ...
          tryCatch(
            paste("raw body:", httr2::resp_body_string(e$resp)),
            error = function(e3) "(no response body available)"
          )
        })
        stop(
          "Supabase export failed for chunk ", i,
          "/", length(row_groups), ": HTTP ", e$status, " -- ",
          body_txt,
          call. = FALSE
        )
      },
