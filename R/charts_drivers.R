...
make_barchart <- function(df_driver) {
  ...
  ggplot(df_driver, aes(x = stringr::str_wrap(Question, wrap), y = percent, fill = Response)) +
    ...
    scale_fill_manual(values = pal_extra, name = NULL) +
    coord_flip() +
...
intention_plot <- ggplot(summary_intention, aes(x = Question, y = percent, fill = Response)) +
  ...
  scale_fill_manual(values = pal_intention, name = NULL) +
  guides(fill = guide_legend(reverse = TRUE))+
...
plots_by_channel <- summary_channel %>%
  ...
  map(~ .x %>%
    dplyr::filter(Response == "Yes") %>% 
      ggplot(aes(x = forcats::fct_reorder(stringr::str_wrap(Question, 40),percent), y = percent, fill = Response)) +
         ...
         scale_fill_manual(values = color_primary_100, name = NULL) +
         guides(fill = guide_legend(reverse = TRUE))+
         coord_flip() + 
         scale_y_continuous(labels = function(x) paste0(x, "%"), expand = expansion(mult = c(0, 0.02))) +
         scale_x_discrete(expand = expansion(add = 0.35)) +
         labs(
           title = unique(.x$Drivers),
           subtitle = unique(.x$long_label),
           x = NULL,
           y = NULL
           ) +
        custom_theme() +
