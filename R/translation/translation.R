...
#   type, key, EN, FR, ES, PT, ..., source_files, usage
#
...
dictionary_file <- "translation.csv"
...
# Language columns are detected from the workbook/CSV headers.  All columns
# other than the metadata columns below are treated as translation languages.
# This means that adding a `PT` column to the source file automatically makes
# Portuguese available through `translation = "PT"` in the configuration.
# A language can be present in either dictionary; missing shared labels fall
# back to EN while a custom workbook label uses its own language column.
.translation_metadata_columns <- c(
  "type", "key", "section", "source_files", "usage"
)

.standardise_translation_columns <- function(dictionary) {
  headers <- trimws(names(dictionary))
  metadata_index <- match(
    tolower(headers),
    .translation_metadata_columns,
    nomatch = 0L
  )
  
  headers[metadata_index > 0L] <-
    .translation_metadata_columns[metadata_index[metadata_index > 0L]]
  
  language_index <- metadata_index == 0L
  headers[language_index] <- toupper(headers[language_index])
  
  if (anyDuplicated(headers)) {
    duplicated_headers <- unique(headers[duplicated(headers)])
    stop(
      "Duplicated translation column(s): ",
      paste(duplicated_headers, collapse = ", "),
      call. = FALSE
    )
  }
  
  names(dictionary) <- headers
  dictionary
}

.detect_translation_languages <- function(dictionary) {
  languages <- setdiff(names(dictionary), .translation_metadata_columns)
  
  if (length(languages) == 0L) {
    stop(
      "The translation dictionary does not contain any language columns.",
      call. = FALSE
    )
  }
...
.resolve_translation_languages <- function(dictionary, languages = NULL) {
  available <- .detect_translation_languages(dictionary)
  
  if (is.null(languages)) {
    return(available)
  }
  
  languages <- toupper(trimws(as.character(languages)))
  languages <- languages[!is.na(languages) & nzchar(languages)]
  languages <- unique(languages)
  
  if (length(languages) == 0L) {
    stop("At least one translation language must be provided.", call. = FALSE)
  }
  
  missing_languages <- setdiff(languages, available)
  if (length(missing_languages) > 0L) {
    stop(
      "The translation dictionary is missing language column(s): ",
...
.dictionary_language_column <- function(dictionary, lang, fallback_language) {
  if (lang %in% names(dictionary)) {
    return(lang)
  }
  
  if (fallback_language %in% names(dictionary)) {
...
.normalise_language <- function(
  ...
  if (is.null(supported) || length(supported) == 0L) {
    supported <- fallback
  }
  
  supported <- toupper(trimws(as.character(supported)))
...
load_translation_dictionary <- function(
    file = repo_path(repo, "R", "translation", dictionary_file),
    languages = NULL,
  ...
  dictionary <- .standardise_translation_columns(dictionary)
  languages <- .resolve_translation_languages(dictionary, languages)
...
# Expected columns: key, section, and the same language columns as the main
# translation dictionary (for example EN, FR, ES and PT).
...
load_custom_translation_dictionary <- function(
  ...
  empty_dictionary <- function() {
    ...
    for (lang in languages) out[[lang]] <- character(0)
...
# A language may be supplied by the shared dictionary, the optional workbook,
# or both. This lets a country add a new column such as PT in its workbook
# without having to duplicate every shared EN/FR/ES entry.
translation_languages <- unique(c(
  attr(translation_dictionary, "languages"),
  attr(custom_translation_dictionary, "languages")
))
attr(translation_dictionary, "languages") <- translation_languages
attr(custom_translation_dictionary, "languages") <- translation_languages
...
.translate_key_from_dictionary <- function(
  ...
  if (is.null(languages)) {
    languages <- .detect_translation_languages(dictionary)
  }
  ...
  value_column <- .dictionary_language_column(
    dictionary,
    lang,
    fallback_language
  )
  fallback_column <- .dictionary_language_column(
    dictionary,
    fallback_language,
    fallback_language
  )
  ...
  idx <- idx[1L]
  value <- dictionary[[value_column]][idx]
  
  if (.is_blank_translation(value)) {
    value <- dictionary[[fallback_column]][idx]
  }
...
.translate_value_from_dictionary <- function(
  ...
  out <- vapply(
    ...
    function(i) {
      ...
      for (source_lang in intersect(languages, names(pool))) {
      ...
      target_column <- .dictionary_language_column(
        pool,
        target_lang,
        attr(dictionary, "fallback_language")
      )
      fallback_column <- .dictionary_language_column(
        pool,
        attr(dictionary, "fallback_language"),
      ...
      translated <- pool[[target_column]][matches[1L]]
      
      if (.is_blank_translation(translated)) {
        translated <- pool[[fallback_column]][matches[1L]]
      }
